    ...
    if mode == "train":
        ...
        data_loader = data.DataLoader(dataset=dataset,
                                      ...
                                      batch_sampler=data.sampler.BatchSampler(sampler=initial_sampler,
                                                                              ...
                                                                              drop_last=False))
    # Being in val mode doesn't ensure pad_limit length of captions
    else:
        data_loader = data.DataLoader(dataset=dataset,
                                      batch_size=dataset.batch_size,
                                      shuffle=True,
...
class CoCoDataset(data.Dataset):
    ...
    def __init__(self, transform, mode, batch_size, vocab_threshold, vocab_file, start_word,
                 ...
                 fetch_mode='default', pad_caption=True, pad_limit=20, data_mode='default',
                 disp_mode='default',test_size=1000):
        self.test_size = test_size
        # Display mode necessary for retrieval recall score task
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "val" and self.disp_mode=="imgcapretrieval" and self.data_mode=="imagecaption":
            ...
            # Will it be 10 or len(image_ids)
            # Get a shuffled list of
            img_random = random.sample(list(np.arange(len(image_ids))),10)
            allimages = []
            allgtcaptions = []
            caption = list()
            all_captions_glove=[]
